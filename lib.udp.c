...
void bind_to(options_t *o) {
    ...
    int status = bind(o->fd, (struct sockaddr*)&addr, sizeof(addr));
    if (status == -1) die("bind");
}
...
int send_data(options_t *o, short *data) {
    int status = sendto(o->fd, data, N, 0, (struct sockaddr*)&o->addr, o->len);
    if (status == -1) perror("sendto");
    return status;
}

int recv_data(options_t *o, sox_format_t *ft) {
    char data[N];
    int n = recvfrom(o->fd, data, N, 0, (struct sockaddr*)&o->addr, &o->len);
    if (n == 0) {
        return -1;
    } else if (n == -1) {
        perror("recvfrom");
...
void wait_client(options_t *o) {
    ...
    int status = select(o->fd + 1, &rfds, NULL, NULL, &tv);

    if (status == -1) die("select");
    else if (status == 0) err("60秒間応答がありませんでした");
...
void* async_send(void* _o) {
    ...
    while (TRUE) {
        ...
        // ノイズ除去(Macの内蔵マイクには環境ノイズリダクション機能がある)
        #ifndef __APPLE__
        // filter(data, o->f1, o->f2);
        #endif

        status = send_data(o, data);
        if (status == -1) {
            o->quit = TRUE;
            break;
        }
